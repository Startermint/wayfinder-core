...
PHP);
        ...
        self::assertStringContainsString('Move DB access into a Model first, use DB only when low-level control is intentional, or escalate to a Query or Service when complexity requires it.', $stderr);
        self::assertStringContainsString('docs/architecture-review-guide.md', $stderr);
    }
...
PHP);
    ...
    public function testCommandIgnoresDbAccessInsideModelsQueriesAndServices(): void
    {
        mkdir($this->tempDir . '/app/Models', 0777, true);
        mkdir($this->tempDir . '/app/Queries', 0777, true);
        mkdir($this->tempDir . '/app/Services', 0777, true);

        file_put_contents($this->tempDir . '/app/Models/Task.php', <<<'PHP'
<?php
...
final class Task
{
    ...
    {
        return DB::raw('SELECT * FROM tasks');
    }
}
PHP);

        file_put_contents($this->tempDir . '/app/Queries/TaskReportQuery.php', <<<'PHP'
<?php

use Wayfinder\Database\DB;

final class TaskReportQuery
{
    public function execute(): array
    {
        return DB::raw('SELECT COUNT(*) AS total FROM tasks');
    }
}
PHP);

        file_put_contents($this->tempDir . '/app/Services/SyncTasksService.php', <<<'PHP'
<?php

use Wayfinder\Database\DB;

final class SyncTasksService
{
    public function execute(): int
    {
        return DB::statement('DELETE FROM task_sync_log');
    }
}
PHP);
