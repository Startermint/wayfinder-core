...
use Wayfinder\Console\MakeServiceCommand;
...
final class MakeDataClassCommandsTest extends TestCase
{
    ...
    public function testCommandsCanBeRegistered(): void
    {
        ...
        self::assertStringContainsString('make:service', $output);
    ...
    public function testMakeServiceCreatesModuleLevelClassWithExpectedStub(): void
    {
        $command = $this->makeServiceCommand();

        ob_start();
        $exit = $command->handle(['SyncInvoices', '--module=Billing', '--namespace=Stripe']);
        ob_end_clean();

        $file = $this->tempDir . '/Modules/Billing/Services/Stripe/SyncInvoicesService.php';

        self::assertSame(0, $exit);
        self::assertFileExists($file);

        $contents = (string) file_get_contents($file);
        self::assertStringContainsString('namespace Modules\\Billing\\Services\\Stripe;', $contents);
        self::assertStringContainsString('final class SyncInvoicesService', $contents);
        self::assertStringContainsString('public function execute(): mixed', $contents);
    }
    ...
    private function makeServiceCommand(): MakeServiceCommand
    {
        return new MakeServiceCommand($this->tempDir . '/app', 'App', $this->tempDir . '/Modules', 'Modules');
    }
