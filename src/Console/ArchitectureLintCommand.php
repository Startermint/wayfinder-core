...
final class ArchitectureLintCommand implements Command
{
    ...
    {
        return 'Scan app and module PHP code for forbidden direct database usage outside the data layer.';
    }
    ...
    public function handle(array $arguments = []): int
    {
        ...
        foreach ($violations as $violation) {
            fwrite(
                $this->stderr,
                sprintf(
                    "  %s:%d matches %s. Move DB access into a Model first, use DB only when low-level control is intentional, or escalate to a Query or Service when complexity requires it.\n",
                    $violation['file'],
                    $violation['line'],
                    $violation['pattern'],
                ),
        ...
        fwrite(
            $this->stderr,
            "See docs/architecture-review-guide.md for Model vs DB vs Query vs Service boundary rules when that guide is available in the project.\n",
        );
