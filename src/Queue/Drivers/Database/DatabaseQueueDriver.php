...
namespace Wayfinder\Queue\Drivers\Database;
...
final class DatabaseQueueDriver implements QueueConnection
{
    ...
    public function reserve(?string $queue = null): ?QueuedJob
    {
        ...
        return $this->database->transaction(function () use ($queue, $now, $reservedCutoff): ?QueuedJob {
            $row = $this->database->firstResult($this->reserveSql(), [$queue, $now, $reservedCutoff]);
    ...
    private function reserveSql(): string
    {
        $sql = sprintf(
        ...
        if (in_array($this->database->driver(), ['mysql', 'pgsql'], true)) {
